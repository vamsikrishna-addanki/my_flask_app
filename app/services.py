...
def import_data():
    ...
    with app.app_context():
        ...
        stores_csv = pd.read_csv('data/store status.csv', chunksize=batch_size)
        business_hours_csv = pd.read_csv('data/Menu hours.csv', chunksize=batch_size)
        ...
        for i, row in stores_df.iterrows():
            ...
            timestamp_local = parse_datetime(row['timestamp_utc']).astimezone(timezone)
            store = Store(timestamp_utc=parse_datetime(row['timestamp_utc']), status=status)
